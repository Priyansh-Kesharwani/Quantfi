      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-effect rounded-sm p-6" data-testid="backtest-config">
          ...
          <div className="space-y-4">
            {/* Asset — using shared AssetPicker */}
            <AssetPicker
              assets={assetList}
              value={config.symbol}
              onChange={(symbol) => setConfig({ ...config, symbol })}
              label="ASSET"
              testId="backtest-asset-select"
            />
            ...
            <div>
              ...
              <input type="number" value={config.dcaAmount} onChange={(e) => setConfig({ ...config, dcaAmount: e.target.value })} className="w-full p-3 glass-effect rounded text-sm font-data" data-testid="backtest-dca-amount" />
            </div>

            {/* Cadence */}
            <div>
              <label className="text-sm text-muted-foreground mb-2 block">DCA CADENCE</label>
              <select value={config.dcaCadence} onChange={(e) => setConfig({ ...config, dcaCadence: e.target.value })} className="w-full p-3 glass-effect rounded text-sm font-data" data-testid="backtest-cadence-select">
            ...
            <div>
              ...
              <input type="number" value={config.buyDipThreshold} onChange={(e) => setConfig({ ...config, buyDipThreshold: e.target.value })} className="w-full p-3 glass-effect rounded text-sm font-data" min="0" max="100" data-testid="backtest-dip-threshold" />
              <p className="text-xs text-muted-foreground mt-1">Invest extra 50% when composite score ≥ this threshold</p>
            </div>

            {/* ─── Execution Cost Toggle (Phase 3) ─── */}
            <div className="border-t border-white/10 pt-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  onClick={() => setConfig(prev => ({ ...prev, enableExecCosts: !prev.enableExecCosts }))}
                ...
                >
                  <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition ${config.enableExecCosts ? 'left-5.5' : 'left-0.5'}`} style={{ left: config.enableExecCosts ? '22px' : '2px' }} />
                </button>
              </div>
              ...
              {config.enableExecCosts && (
                <div className="space-y-3 pl-6" data-testid="exec-costs-config">
                  <div>
                    ...
                    <input type="number" value={config.slippageBps} onChange={(e) => setConfig({ ...config, slippageBps: e.target.value })} className="w-full p-2 glass-effect rounded text-xs font-data" min="0" max="100" data-testid="slippage-input" />
                  </div>
                  <div>
                    <label className="text-[10px] text-muted-foreground block mb-1">IMPACT GAMMA (0-1)</label>
                    <input type="number" value={config.impactGamma} onChange={(e) => setConfig({ ...config, impactGamma: e.target.value })} className="w-full p-2 glass-effect rounded text-xs font-data" min="0" max="1" step="0.1" data-testid="impact-gamma-input" />
                  </div>
                  <div>
                    <label className="text-[10px] text-muted-foreground block mb-1">TXN COST (BPS)</label>
                    <input type="number" value={config.txnCostBps} onChange={(e) => setConfig({ ...config, txnCostBps: e.target.value })} className="w-full p-2 glass-effect rounded text-xs font-data" min="0" max="100" data-testid="txn-cost-input" />
                  </div>
            ...
            {/* Run button */}
            <button onClick={runBacktest} disabled={running} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-primary text-primary-foreground rounded font-medium hover:bg-primary/90 transition disabled:opacity-50" data-testid="run-backtest-btn">
          ...
          {!result ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4" data-testid="backtest-results">
                ...
                <StatCard
                  icon={TrendingUp}
                  label="Final Value (USD)"
                  value={formatCurrency(result.final_value_usd, 'USD')}
                ...
                <StatCard
                  ...
                  value={formatPercent(result.annualized_return_pct, 2)}
                  ...
                  <div style={{ width: '100%', height: 380 }}>
                    <ResponsiveContainer>
                      <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                        ...
                        <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#888' }} tickFormatter={(d) => d?.substring(0, 7)} interval={Math.max(1, Math.floor(chartData.length / 8))} />
                        ...
                        <Line type="monotone" dataKey="dipBuy" name="Dip Buy" stroke="none" dot={{ fill: '#FBBF24', r: 4, stroke: '#F59E0B', strokeWidth: 2 }} activeDot={{ r: 7, fill: '#FBBF24', stroke: '#fff', strokeWidth: 2 }} legendType="diamond" connectNulls={false} isAnimationActive={false} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </div>
                </div>
