          ...
          {filtered.map((article, index) => (
            <div key={article.id || index} className="glass-effect rounded-sm p-5 hover:border-primary/30 transition" data-testid={`news-article-${index}`}>
              <div className="flex gap-3">
                <div className="flex-shrink-0 text-muted-foreground mt-0.5">{getEventIcon(article.event_type)}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-1">
                    <h3 className="text-sm font-bold leading-tight" data-testid={`news-title-${index}`}>{article.title}</h3>
                    <span className="text-[10px] text-muted-foreground whitespace-nowrap shrink-0">{formatAge(article.published_at)}</span>
                  </div>
                  <p className="text-xs text-muted-foreground mb-2 line-clamp-2">{article.summary || article.description}</p>
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-[10px] px-1.5 py-0.5 glass-effect rounded font-bold">{(article.event_type || 'general').replace('_', ' ').toUpperCase()}</span>
                      {article.source && <span className="text-[10px] text-muted-foreground">{article.source}</span>}
                      {(article.affected_assets || []).map(asset => (
                        <span key={asset} className="text-[10px] px-1.5 py-0.5 bg-primary/10 text-primary rounded font-data font-bold">{asset}</span>
                      ))}
                    </div>
                    {article.url && (
                      <a href={article.url} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1 text-[10px] text-primary hover:underline shrink-0" data-testid={`news-link-${index}`}>
