        ...
        <div style={{ width: 180, height: 180 }}>
          <ResponsiveContainer>
            <RadialBarChart
              ...
              data={radarData} startAngle={180} endAngle={0} barSize={12}
            >
              <RadialBar background={{ fill: 'rgba(255,255,255,0.03)' }} dataKey="score" cornerRadius={4} />
            </RadialBarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="relative">
          <svg width="100" height="100" viewBox="0 0 100 100">
            ...
            <circle cx="50" cy="50" r="42" fill="none" stroke={healthColor} strokeWidth="8"
              strokeLinecap="round" strokeDasharray={`${(health / 100) * 264} 264`}
              transform="rotate(-90 50 50)" style={{ transition: 'stroke-dasharray 0.8s ease' }} />
            <text x="50" y="46" textAnchor="middle" fill={healthColor} fontSize="22" fontWeight="bold" fontFamily="JetBrains Mono">{health}</text>
            <text x="50" y="62" textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize="8" fontWeight="600">{healthLabel}</text>
          </svg>
        </div>
      ...
      <div style={{ width: '100%', height: 220 }}>
        <ResponsiveContainer>
          <BarChart data={chartData} margin={{ top: 5, right: 5, left: -10, bottom: 5 }}>
            <XAxis dataKey="symbol" tick={{ fontSize: 10, fill: '#888' }}
...
const SmartAlerts = ({ assets }) => {
  const alerts = useMemo(() => {
    ...
    assets.forEach(item => {
      ...
      if (zone === 'strong_buy') {
        result.push({
          type: 'opportunity', symbol: item.asset.symbol,
          message: `Strong buy zone — DCA score ${score?.toFixed(0)}`,
          icon: Flame, color: 'text-emerald-400 bg-emerald-400/10',
        });
      }
      if (rsi != null && rsi < 30) {
        result.push({
          type: 'oversold', symbol: item.asset.symbol,
          message: `RSI ${rsi.toFixed(1)} — Oversold territory`,
          icon: Activity, color: 'text-blue-400 bg-blue-400/10',
        });
      }
      if (drawdown != null && drawdown < -20) {
        result.push({
          type: 'drawdown', symbol: item.asset.symbol,
          message: `${drawdown.toFixed(1)}% drawdown from highs`,
          icon: TrendingDown, color: 'text-red-400 bg-red-400/10',
        });
      }
    });
  ...
  return (
    <div className="glass-effect rounded-sm p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full bg-primary/20 text-primary font-bold">{alerts.length}</span>
      </div>
            ...
            {topAssets.map(item => {
              ...
              return (
                <tr key={item.asset.symbol}
                  ...
                  <td className="text-right py-3"><ZoneBadge zone={zone} size="sm" /></td>
                  <td className="text-right py-3">
...
const Dashboard = () => {
  ...
  const metrics = useMemo(() => {
    let totalUSD = 0, totalINR = 0, scoreSum = 0, scoreCount = 0;
    let bestAsset = null, worstAsset = null, strongBuyCount = 0, favorableCount = 0;

    dashboardData.forEach(item => {
      totalUSD += item.price?.price_usd || 0;
      totalINR += item.price?.price_inr || 0;
      const score = item.score?.composite_score;
      if (score != null) {
        scoreSum += score;
        scoreCount++;
    ...
    return { totalUSD, totalINR, avgScore, scoreCount, bestAsset, worstAsset, strongBuyCount, favorableCount, totalAssets: dashboardData.length };
  }, [dashboardData]);
      ...
      <div className="flex items-center justify-center h-screen" data-testid="error-dashboard">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Make sure the backend is running at {import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'}.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <button className="px-6 py-3 border border-white/20 rounded font-medium hover:bg-white/5 transition"
    ...
    return (
      <div className="flex items-center justify-center h-screen" data-testid="empty-dashboard">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-2">Add assets to your watchlist to start analyzing DCA opportunities.</p>
          <p className="text-sm text-muted-foreground mb-6">Click the button below or use &quot;+ ADD ASSET&quot; in the sidebar.</p>
          <button className="px-6 py-3 bg-primary text-primary-foreground rounded font-medium hover:bg-primary/90 transition"
            onClick={() => window.dispatchEvent(new CustomEvent('addAsset'))} data-testid="add-first-asset-btn">
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1600px] mx-auto" data-testid="dashboard-main">
      <div className="flex items-center justify-between mb-6">
        ...
        <RefreshButton onClick={refresh} loading={refreshing} testId="refresh-dashboard-btn" />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard icon={Wallet} label="Portfolio Snapshot" value={fmt(metrics.totalUSD)} subValue={fmtINR(metrics.totalINR)} />
        <StatCard icon={Gauge} label="Avg DCA Score" value={metrics.avgScore.toFixed(1)}
          subValue={`${metrics.scoreCount} assets scored`} trend={metrics.avgScore >= 50 ? 'Favorable' : 'Below avg'} trendPositive={metrics.avgScore >= 50} />
        <StatCard icon={TrendingUp} label="Best Opportunity" value={metrics.bestAsset?.symbol || '—'}
          subValue={metrics.bestAsset ? `Score: ${metrics.bestAsset.score.toFixed(0)}` : 'N/A'}
          trendPositive={true} trend={metrics.bestAsset?.score >= 60 ? 'Buy zone' : null} />
        <StatCard icon={Activity} label="Total Assets" value={metrics.totalAssets} subValue={`${metrics.strongBuyCount} in buy zone`} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2"><PerformanceChart assets={dashboardData} /></div>
        <div className="lg:col-span-1"><RecentNewsMini /></div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        <OpportunityRadar assets={dashboardData} />
        <PortfolioHealth avgScore={metrics.avgScore} strongBuyCount={metrics.strongBuyCount}
          favorableCount={metrics.favorableCount} totalCount={metrics.totalAssets} />
