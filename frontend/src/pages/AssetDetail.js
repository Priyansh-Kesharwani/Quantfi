              ...
              <div className="space-y-2">
                {sentiment.top_factors.map((f, i) => (
                  <div key={i} className="flex items-start gap-3 p-3 rounded bg-white/[0.02] border border-white/5">
                    <div className="text-lg font-bold font-data text-muted-foreground w-6 text-center shrink-0">
                      {f.rank || i + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span className="text-[10px] text-muted-foreground font-data">
                          impact: {((f.impact_magnitude ?? 0) * 100).toFixed(0)}%
                        </span>
                      </div>
                      {f.explanation && (
                      ...
                      {f.supporting_sources?.length > 0 && (
                        <div className="flex gap-1.5 mt-1.5 flex-wrap">
                          {f.supporting_sources.map((s, j) => (
                            <TierBadge key={j} sourceId={s} />
                          ))}
                        </div>
                      )}
                    </div>
          ...
          <div>
            ...
            {price ? (
              <div className="flex items-baseline gap-3 flex-wrap mt-3">
                <span className="text-3xl font-data font-bold whitespace-nowrap" data-testid="asset-detail-price">
                  {formatCurrency(price.price_usd, 'USD')}
                </span>
                <span className="text-xl font-data text-muted-foreground whitespace-nowrap">
