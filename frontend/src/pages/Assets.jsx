...
import { formatCurrency, getScoreColor } from '@/utils';
import {
...
const AssetGridCard = ({ item, onNavigate, onRemove }) => {
  ...
  return (
    <div className="glass-effect rounded-sm p-5 hover:border-primary/30 transition-all cursor-pointer group relative"
      onClick={() => onNavigate(`/assets/${asset.symbol}`)} data-testid={`asset-card-${asset.symbol}`}>
      <button onClick={(e) => { e.stopPropagation(); onRemove(asset.symbol); }}
        ...
        <>
          <div className="mb-3">
            <div className="flex items-baseline gap-2 flex-wrap">
              <span className="text-xl font-data font-semibold whitespace-nowrap">{formatCurrency(price?.price_usd, 'USD')}</span>
              {price?.price_inr != null && (
                <span className="text-sm text-muted-foreground font-data whitespace-nowrap">{formatCurrency(price?.price_inr, 'INR')}</span>
              )}
            </div>
          </div>
...
const AssetTableRow = ({ item, onNavigate, onRemove }) => {
  ...
  return (
    <tr className="border-b border-white/[0.04] hover:bg-white/[0.03] cursor-pointer transition group"
      onClick={() => onNavigate(`/assets/${asset.symbol}`)} data-testid={`asset-row-${asset.symbol}`}>
      <td className="py-4 pl-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-white/5 flex items-center justify-center text-xs font-bold font-data text-primary">{asset.symbol.slice(0, 2)}</div>
          <div>
      ...
      <td className="py-4 text-right">
        <span className="text-[10px] px-2 py-0.5 rounded font-bold bg-white/5 text-muted-foreground uppercase">{asset.asset_type}</span>
      </td>
      <td className="py-4 text-right">
        <div className="text-sm font-data">{formatCurrency(price?.price_usd, 'USD')}</div>
        {price?.price_inr != null && <div className="text-xs text-muted-foreground font-data">{formatCurrency(price?.price_inr, 'INR')}</div>}
      </td>
      <td className="py-4 text-right">
        <div className="flex items-center justify-end gap-2">
          ...
          <span className="text-sm font-data font-bold w-8 text-right" style={{ color: getScoreColor(scoreValue) }}>{scoreValue.toFixed(0)}</span>
        </div>
      </td>
      <td className="py-4 text-right"><ZoneBadge zone={zone} size="sm" /></td>
      <td className="py-4 text-right font-data text-sm">{indicators?.rsi_14 != null ? indicators.rsi_14.toFixed(1) : '—'}</td>
      <td className="py-4 text-right font-data text-sm">
        {indicators?.drawdown_pct != null ? (
          <span className={indicators.drawdown_pct < -10 ? 'text-red-400' : ''}>{indicators.drawdown_pct.toFixed(1)}%</span>
        ) : '—'}
      </td>
      <td className="py-4 pr-4 text-right">
        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition">
          <button onClick={(e) => { e.stopPropagation(); onNavigate(`/assets/${asset.symbol}`); }}
            className="p-1.5 rounded hover:bg-white/10 text-muted-foreground hover:text-primary transition" title="View details">
            <ExternalLink className="w-3.5 h-3.5" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); onRemove(asset.symbol); }}
            className="p-1.5 rounded hover:bg-destructive/20 text-muted-foreground hover:text-destructive transition" title="Remove">
            <Trash2 className="w-3.5 h-3.5" />
          </button>
        </div>
      </td>
...
const Assets = () => {
  ...
  const filteredAssets = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(item => item.asset.symbol.toLowerCase().includes(q) || (item.asset.name || '').toLowerCase().includes(q));
    }
    if (categoryFilter !== 'all') result = result.filter(item => item.asset.asset_type === categoryFilter);
    if (zoneFilter !== 'all') result = result.filter(item => item.score?.zone === zoneFilter);
    const [key, dir] = sortBy.split('_');
    result.sort((a, b) => {
      let aVal, bVal;
      switch (key) {
        case 'score': aVal = a.score?.composite_score ?? 0; bVal = b.score?.composite_score ?? 0; break;
        case 'name': aVal = a.asset.symbol; bVal = b.asset.symbol; return dir === 'asc' ? aVal.localeCompare(bVal) : bVal.localeCompare(aVal);
        case 'price': aVal = a.price?.price_usd ?? 0; bVal = b.price?.price_usd ?? 0; break;
        case 'rsi': aVal = a.indicators?.rsi_14 ?? 999; bVal = b.indicators?.rsi_14 ?? 999; break;
        case 'drawdown': aVal = a.indicators?.drawdown_pct ?? 0; bVal = b.indicators?.drawdown_pct ?? 0; break;
  ...
  const activeFiltersCount = [categoryFilter !== 'all' ? 1 : 0, zoneFilter !== 'all' ? 1 : 0, searchQuery.trim() ? 1 : 0].reduce((a, b) => a + b, 0);
    ...
    return (
      <div className="flex items-center justify-center h-screen" data-testid="empty-assets">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-6">Add assets to your watchlist to start analyzing DCA opportunities.</p>
          <button className="px-6 py-3 bg-primary text-primary-foreground rounded font-medium hover:bg-primary/90 transition"
            onClick={() => window.dispatchEvent(new Event('addAsset'))} data-testid="add-first-asset-btn">
    ...
    <div className="p-6 lg:p-8 max-w-[1600px] mx-auto" data-testid="assets-page">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            ...
            {filteredAssets.length !== dashboardData.length && <span className="text-primary ml-1">· {filteredAssets.length} shown</span>}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => window.dispatchEvent(new Event('addAsset'))}
      ...
      {viewMode === 'grid' && filteredAssets.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5" data-testid="asset-grid">
          {filteredAssets.map(item => <AssetGridCard key={item.asset.symbol} item={item} onNavigate={navigate} onRemove={handleRemoveAsset} />)}
        </div>
      )}
        ...
        <div className="glass-effect rounded-sm overflow-hidden" data-testid="asset-table">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {filteredAssets.map(item => <AssetTableRow key={item.asset.symbol} item={item} onNavigate={navigate} onRemove={handleRemoveAsset} />)}
              </tbody>
            </table>
          </div>
        </div>
