          ...
          <div className="flex gap-1 p-2 border-b flex-wrap">
            {DATE_PRESETS.map(p => (
              <Button
                ...
                onClick={() => {
                  onDateRangeChange({ from: subDays(new Date(), p.days), to: new Date() });
                }}
              >
                {p.label}
              </Button>
          ...
          <Calendar
            ...
            onSelect={(range) => {
              if (range?.from && range?.to) {
                ...
                onDateRangeChange({ from: range.from, to: dateRange.to });
              }
            }}
            numberOfMonths={2}
...
function EquityChart({ result, initialCapital, trades = [] }) {
  ...
  const {
    ...
    // Left Y labels (equity, nice round numbers)
    ...
    // Trade entry/exit markers mapped onto equity chart
    const tradeMarkers = [];
  ...
  const totalRetPct = ((finalEquity - initialCapital) / initialCapital * 100).toFixed(1);
    ...
    <Card className="glass-effect">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            {bhPathD && (
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-0.5 bg-gray-500 rounded" style={{ borderTop: '1px dashed #888' }} />
      ...
      <CardContent className="pt-0">
        ...
        >
          ...
          {regimes.length > 0 && (() => {
            const bands = [];
            let curRegime = regimes[0], start = 0;
            for (let i = 1; i <= regimes.length; i++) {
              if (i === regimes.length || regimes[i] !== curRegime) {
                const x1 = toX(start);
                const x2 = toX(Math.min(i, pts.length - 1));
                if (regimeColors[curRegime]) {
                  bands.push(
                    <rect key={`r${start}`} x={x1} y={padT} width={x2 - x1} height={h - padT - padB}
                      fill={regimeColors[curRegime]} />
                  );
                }
                if (i < regimes.length) { curRegime = regimes[i]; start = i; }
              }
            }
            return bands;
          })()}
          ...
          {/* Gridlines */}
          {yLabels && yLabels.map((yl, i) => (
            <g key={`yl${i}`}>
              <line x1={padL} y1={yl.y} x2={w - padR} y2={yl.y} stroke="#333" strokeWidth="0.5" />
              <text x={padL - 6} y={yl.y + 3.5} fill="#888" fontSize="9" textAnchor="end" fontFamily="monospace">{yl.label}</text>
            </g>
          ))}

          {/* X axis date labels */}
          {dateLabels && dateLabels.map((dl, i) => (
            <text key={`dl${i}`} x={dl.x} y={h - padB + 16} fill="#888" fontSize="9" textAnchor="middle" fontFamily="monospace">{dl.label}</text>
          ))}

          {/* Initial capital baseline */}
          <line x1={padL} y1={toY(initialCapital)} x2={w - padR} y2={toY(initialCapital)}
          ...
          {/* Asset price line (right axis) */}
          {pricePathD && <path d={pricePathD} fill="none" stroke="#a78bfa" strokeWidth="1" strokeOpacity="0.7" />}

          {/* Right Y-axis labels (price) */}
          {priceYLabels && priceYLabels.map((pl, i) => (
            <g key={`pl${i}`}>
              <line x1={w - padR} y1={pl.y} x2={w - padR + 4} y2={pl.y} stroke="#a78bfa" strokeWidth="0.5" strokeOpacity="0.5" />
              <text x={w - padR + 7} y={pl.y + 3.5} fill="#a78bfa" fontSize="8" textAnchor="start" fontFamily="monospace" opacity="0.7">{pl.label}</text>
            </g>
          ))}
          ...
          {/* Equity line (green above initial, red below) */}
          <path d={pathD} fill="none" stroke={finalEquity >= initialCapital ? '#22c55e' : '#ef4444'} strokeWidth="1.5" />

          {/* Peak marker */}
          {peakIdx !== undefined && (
            <circle cx={toX(peakIdx)} cy={toY(peakVal)} r="3" fill="#22c55e" stroke="#fff" strokeWidth="0.5" />
          )}

          {/* Trade entry/exit markers */}
          {tradeMarkers && tradeMarkers.map((m, i) => (
            <g key={`tm${i}`}>
              {m.isEntry ? (
                <polygon
          ...
          {tooltip && (
            <>
              ...
              {tooltip.price && (
                <text x={tooltip.x + 14} y={tooltip.y - 4} fill="#a78bfa" fontSize="9" fontFamily="monospace">
                  Price: {tooltip.price}
                </text>
              )}
              <text x={tooltip.x + 14} y={tooltip.price ? tooltip.y + 9 : tooltip.y - 3} fill={parseFloat(tooltip.returnPct) >= 0 ? '#22c55e' : '#ef4444'} fontSize="9" fontFamily="monospace">
                {parseFloat(tooltip.returnPct) >= 0 ? '+' : ''}{tooltip.returnPct}%
                {tooltip.regime && ` · ${tooltip.regime}`}
              </text>
            </>
          )}
        </svg>
...
const SIDE_LABELS = {
  ...
  grid_buy: 'Grid Buy', grid_sell: 'Grid Sell',
};
const SIDE_COLORS = {
  long_entry: 'text-green-400', long_exit: 'text-green-600',
  short_entry: 'text-red-400', short_exit: 'text-red-600',
  grid_buy: 'text-blue-400', grid_sell: 'text-blue-600',
};
...
function TradeTable({ trades = [] }) {
  ...
  const exportCSV = useCallback(() => {
    ...
    const rows = sorted.map(t => cols.map(c => {
      const v = t[c];
  ...
  const SortHeader = ({ label, field, className = '' }) => (
    <th className={`px-2 py-2 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider cursor-pointer select-none hover:text-foreground transition-colors ${className}`}
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-muted-foreground">
              {totals.count} trades · PnL: <span className={totals.totalPnl >= 0 ? 'text-green-400' : 'text-red-400'}>${totals.totalPnl.toFixed(2)}</span>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1">
              ...
              <Select value={sideFilter} onValueChange={v => { setSideFilter(v); setPage(0); }}>
                <SelectTrigger className="h-7 text-xs w-[100px] glass-effect"><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={pnlFilter} onValueChange={v => { setPnlFilter(v); setPage(0); }}>
                <SelectTrigger className="h-7 text-xs w-[100px] glass-effect"><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              {pageRows.map((t, i) => (
                <tr key={i} className="border-b border-border/40 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-2 py-1.5">
                    <span className={`flex items-center gap-1 font-medium ${SIDE_COLORS[t.side] || ''}`}>
                  ...
                  <td className={`px-2 py-1.5 font-mono font-medium ${t.pnl > 0 ? 'text-green-400' : t.pnl < 0 ? 'text-red-400' : 'text-muted-foreground'}`}>
                    {t.pnl === 0 ? '—' : `${t.pnl > 0 ? '+' : ''}$${t.pnl.toFixed(2)}`}
                  </td>
                  <td className="px-2 py-1.5 font-mono text-muted-foreground">${t.fee?.toFixed(2)}</td>
                  <td className="px-2 py-1.5">
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-mono ${
                      t.regime === 'TRENDING' ? 'bg-blue-500/20 text-blue-400' :
                      t.regime === 'STRESS' ? 'bg-red-500/20 text-red-400' :
        ...
        {pageCount > 1 && (
          <div className="flex items-center justify-between mt-3 pt-2 border-t border-border">
            ...
            <div className="flex gap-1">
              <Button variant="outline" size="sm" className="h-6 px-2 text-xs" disabled={page === 0}
                onClick={() => setPage(p => p - 1)}>Prev</Button>
              <Button variant="outline" size="sm" className="h-6 px-2 text-xs" disabled={page >= pageCount - 1}
...
export default function CryptoBot() {
  ...
  const handleTimeframeChange = useCallback((tf) => {
    ...
    if (!dateManual) {
      setDateRange(defaultDateRange(tf));
    }
  }, [dateManual]);
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-6">
      {/* Header with asset search */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">CRYPTO TRADING BOT</h1>
            <p className="text-muted-foreground mt-1">
              Directional Futures + Grid Trading with Regime Detection
            </p>
          </div>
          <CryptoAssetSearch
            value={config.symbol}
            onChange={(sym) => updateConfig('symbol', sym)}
          />
        </div>
        <Button onClick={runBacktest} disabled={loading} size="lg" className="min-w-[160px]">
        ...
        <Card className="glass-effect lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Select value={config.strategy_mode} onValueChange={v => updateConfig('strategy_mode', v)}>
                <SelectTrigger className="glass-effect mt-1"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="adaptive">Adaptive (Recommended)</SelectItem>
                  <SelectItem value="directional">Directional Futures</SelectItem>
            ...
            <div>
              <Label className="text-xs text-muted-foreground">LEVERAGE: {config.leverage}x</Label>
              <Slider value={[config.leverage]} min={1} max={10} step={1}
                onValueChange={([v]) => updateConfig('leverage', v)} className="mt-2" />
            </div>
            ...
            <div>
              <Label className="text-xs text-muted-foreground">ENTRY THRESHOLD: {config.entry_threshold}</Label>
              <Slider value={[config.entry_threshold]} min={10} max={60} step={5}
                onValueChange={([v]) => updateConfig('entry_threshold', v)} className="mt-2" />
            </div>

            <div>
              <Label className="text-xs text-muted-foreground">EXIT THRESHOLD: {config.exit_threshold}</Label>
              <Slider value={[config.exit_threshold]} min={5} max={30} step={5}
                onValueChange={([v]) => updateConfig('exit_threshold', v)} className="mt-2" />
            </div>

            <div>
              <Label className="text-xs text-muted-foreground">ATR TRAIL: {config.atr_trail_mult}x</Label>
              <Slider value={[config.atr_trail_mult]} min={2.0} max={6.0} step={0.5}
                onValueChange={([v]) => updateConfig('atr_trail_mult', v)} className="mt-2" />
            </div>

            <div>
              <Label className="text-xs text-muted-foreground">RISK PER TRADE: {(config.max_risk_per_trade * 100).toFixed(0)}%</Label>
              <Slider value={[config.max_risk_per_trade]} min={0.05} max={0.40} step={0.01}
                onValueChange={([v]) => updateConfig('max_risk_per_trade', v)} className="mt-2" />
            </div>

            <div>
              <Label className="text-xs text-muted-foreground">KELLY FRACTION: {config.kelly_fraction}</Label>
              <Slider value={[config.kelly_fraction]} min={0.1} max={0.5} step={0.05}
                onValueChange={([v]) => updateConfig('kelly_fraction', v)} className="mt-2" />
            </div>

            <div>
              <Label className="text-xs text-muted-foreground">GRID LEVELS: {config.grid_levels}</Label>
              <Slider value={[config.grid_levels]} min={5} max={40} step={5}
                onValueChange={([v]) => updateConfig('grid_levels', v)} className="mt-2" />
            </div>

            <div>
              <Label className="text-xs text-muted-foreground">GRID ORDER SIZE: ${config.grid_order_size}</Label>
              <Slider value={[config.grid_order_size]} min={50} max={500} step={50}
                onValueChange={([v]) => updateConfig('grid_order_size', v)} className="mt-2" />
            </div>
          </CardContent>
        </Card>
            ...
            <>
              ...
              {result.data_source && (
                <div className="flex items-center gap-2 text-xs">
                  ...
                  {result.n_bars_actual > 0 && <span className="text-muted-foreground">{result.n_bars_actual.toLocaleString()} bars</span>}
                  {result.start_date && result.end_date && (
                    <span className="text-muted-foreground">{result.start_date} — {result.end_date}</span>
                  )}
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className="glass-effect">
                  ...
                  <CardContent className="space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-muted-foreground">Total Fees</span><span>${result.total_fees.toFixed(2)}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Total Funding</span><span>${result.total_funding.toFixed(2)}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Avg Trade PnL</span><span className={result.avg_trade_pnl > 0 ? 'text-green-400' : 'text-red-400'}>${result.avg_trade_pnl.toFixed(2)}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Profit Factor</span><span>{result.profit_factor.toFixed(2)}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Sortino</span><span>{result.sortino.toFixed(2)}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Calmar</span><span>{result.calmar.toFixed(2)}</span></div>
                  </CardContent>
                </Card>
                ...
                <Card className="glass-effect">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded bg-blue-500/20 text-blue-400 font-mono">HMM</span>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3 text-sm">
                    {result.regime_counts && (() => {
                      const total = Object.values(result.regime_counts).reduce((a, b) => a + b, 0);
                      const colors = { TRENDING: '#3b82f6', RANGING: '#eab308', STRESS: '#ef4444' };
                      return (
                        <>
                          <div className="flex h-3 rounded-full overflow-hidden gap-0.5">
                            {Object.entries(result.regime_counts).map(([regime, count]) => (
                              <div key={regime} style={{ width: `${(count / total * 100).toFixed(1)}%`, backgroundColor: colors[regime] || '#666' }}
                                className="rounded-sm" title={`${regime}: ${count}`} />
                            ))}
                          </div>
                          {Object.entries(result.regime_counts).map(([regime, count]) => (
                            <div key={regime} className="flex justify-between">
                              <span className={`font-medium ${regime === 'TRENDING' ? 'text-blue-400' : regime === 'RANGING' ? 'text-yellow-400' : 'text-red-400'}`}>
                                {regime}
                              </span>
                              <span>{count} bars ({(count / total * 100).toFixed(0)}%)</span>
                            </div>
                          ))}
                        </>
                      );
                ...
                {/* Baselines */}
                <Card className="glass-effect">
                  <CardHeader><CardTitle className="text-sm">VS BASELINES</CardTitle></CardHeader>
                  <CardContent className="space-y-2 text-sm">
                    {result.baselines && Object.entries(result.baselines).map(([name, metrics]) => (
                    ...
                    <div className="border-t border-border pt-2 mt-2 flex justify-between font-medium">
                      <span>Strategy</span>
                      <span className={result.total_return_pct > 0 ? 'text-green-400' : 'text-red-400'}>
              ...
              {result.score_reachability && (
                <Card className="glass-effect">
                  ...
                  <CardContent className="grid grid-cols-5 gap-4 text-sm">
                    <div><span className="text-muted-foreground block">Mean</span>{result.score_reachability.mean?.toFixed(2)}</div>
                    <div><span className="text-muted-foreground block">Std Dev</span>{result.score_reachability.std?.toFixed(2)}</div>
                    <div><span className="text-muted-foreground block">P5</span>{result.score_reachability.p5?.toFixed(2)}</div>
                    <div><span className="text-muted-foreground block">P95</span>{result.score_reachability.p95?.toFixed(2)}</div>
                    <div>
                      <span className="text-muted-foreground block">Entry %</span>
                      <span className={result.score_reachability.ok ? 'text-green-400' : 'text-red-400'}>
          ...
          {!result && !loading && (
            <Card className="glass-effect">
              <CardContent className="py-20 text-center">
                <p className="text-muted-foreground text-lg">Configure parameters and click RUN BACKTEST to start</p>
                <p className="text-muted-foreground text-sm mt-2">
                  The adaptive strategy switches between Directional Futures (trending markets) and Grid Trading (ranging markets)
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {loading && (
            <Card className="glass-effect">
              <CardContent className="py-20 text-center">
                ...
                <p className="text-muted-foreground mt-4">Running backtest...</p>
              </CardContent>
            </Card>
          )}
        </div>
