        ...
        <Dialog open={showAddAsset} onOpenChange={setShowAddAsset}>
          <DialogContent className="glass-effect z-[100]" data-testid="add-asset-dialog">
            ...
            <div className="space-y-4 mt-4">
              <div>
                <Label htmlFor="symbol" className="text-sm text-muted-foreground mb-2 block">SYMBOL</Label>
                <Input id="symbol" placeholder="Yahoo Finance ticker symbol" value={newAsset.symbol}
                  onChange={(e) => setNewAsset({ ...newAsset, symbol: e.target.value.toUpperCase() })}
                  className="glass-effect" data-testid="asset-symbol-input" />
                <p className="text-xs text-muted-foreground mt-1">Use the ticker symbol as listed on Yahoo Finance.</p>
              </div>

              <div>
                <Label htmlFor="name" className="text-sm text-muted-foreground mb-2 block">NAME</Label>
                <Input id="name" placeholder="e.g., Gold Futures, Apple Inc." value={newAsset.name}
                  onChange={(e) => setNewAsset({ ...newAsset, name: e.target.value })}
                  className="glass-effect" data-testid="asset-name-input" />
              </div>

              <div>
                <Label htmlFor="type" className="text-sm text-muted-foreground mb-2 block">ASSET TYPE</Label>
                <Select value={newAsset.asset_type} onValueChange={(value) => setNewAsset({ ...newAsset, asset_type: value })}>
                  <SelectTrigger className="glass-effect" data-testid="asset-type-select"><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label htmlFor="exchange" className="text-sm text-muted-foreground mb-2 block">EXCHANGE (OPTIONAL)</Label>
                <Input id="exchange" placeholder="Leave blank for auto-detect" value={newAsset.exchange}
                  onChange={(e) => setNewAsset({ ...newAsset, exchange: e.target.value.toUpperCase() })}
                  className="glass-effect" data-testid="asset-exchange-select" />
              </div>

              <Button onClick={handleAddAsset} disabled={adding}
                className="w-full bg-primary text-primary-foreground hover:bg-primary/90" data-testid="confirm-add-asset-btn">
