...
try:
    from engine.scoring.composite import SCORING_RULES as _SR, DEFAULT_WEIGHTS as _DW
except ImportError:
    _SR = None
    _DW = None
...
def _compute_hmm_regime(
    ...
    """Compute per-bar regime labels using rolling HMM from engine.bot.regime."""
    try:
        from engine.bot.regime import regime_probability_rolling
    except ImportError:
...
def prepare_multi_asset_data(
    ...
    PriceProvider = PriceAdapter(get_backend_config())
