...
class TechnicalIndicators:
    ...
    @staticmethod
    def calculate_atr(
        high: pd.Series, low: pd.Series, close: pd.Series, period: Optional[int] = None,
    ) -> pd.Series:
        if period is None:
            period = _cfg().atr_period
        high_low = high - low
    ...
    def calculate_atr_percentile(
        atr: pd.Series, lookback: Optional[int] = None,
    ) -> pd.Series:
        if lookback is None:
            lookback = _cfg().atr_percentile_lookback
