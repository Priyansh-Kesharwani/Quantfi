...
from backend.core.protocols import IFXProvider
...
class SimulationService:
    ...
    def _load_simulator_module() -> Any:
        import engine.backtester.portfolio_simulator as mod
        return mod
    ...
    async def run_simulation(
        ...
        scoring_mode = getattr(
            request, "scoring_mode", tpl.get("scoring_mode", "adaptive")
        )

        sim_mode = getattr(
            request, "simulation_mode", tpl.get("simulation_mode", "tactical")
        )
        ...
        if getattr(request, "export_trades", False) and result.get("trades"):
            ...
            try:
                written = mod.export_trades_csv(
                    str(csv_path), trades=result["trades"], run_id=run_id,
                )
