...
from engine.scoring.composite import (
...
class ScoringEngine:
    ...
    The actual scoring rules live in ``engine.scoring.composite`` so that
    ...
    @staticmethod
    def calculate_volatility_opportunity_score(
        indicators: Dict,
    ) -> Tuple[float, List[str]]:
        factors: List[str] = []
        ...
        if atr_percentile and atr_percentile > 80:
            factors.append(
                f"Volatility at {atr_percentile:.0f}th percentile "
                "(high uncertainty = opportunity)"
            )
    ...
    @staticmethod
    def calculate_statistical_deviation_score(
        indicators: Dict,
    ) -> Tuple[float, List[str]]:
        factors: List[str] = []
        z_scores = []
        ...
        if avg_z is not None:
            if avg_z < -2.0:
                factors.append(
                    f"Price {abs(avg_z):.1f} std deviations below mean "
                    "(extreme statistical dip)"
                )
            elif avg_z < -1.5:
                factors.append(
                    f"Price {abs(avg_z):.1f} std deviations below mean "
                    "(strong statistical dip)"
                )
            elif avg_z < -1.0:
                factors.append(
                    f"Price {abs(avg_z):.1f} std deviations below mean "
                    "(moderate statistical dip)"
                )
    ...
    def calculate_macro_fx_score(
        ...
        if deviation_pct > 5:
            factors.append(
                f"USD-INR at {usd_inr_rate:.2f}, {deviation_pct:.1f}% above average "
                "(expensive for INR buyers)"
            )
        elif deviation_pct < -2:
            factors.append(
                f"USD-INR at {usd_inr_rate:.2f}, favorable for INR buyers"
            )
...
class ScoringFacade:
    ...
    def get_indicators_for_symbol(
        ...
        period = period or getattr(self._config, "history_period", "2y")
