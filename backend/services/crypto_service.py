"""Crypto trading bot service — market catalog, OHLCV, regime detection, backtest."""
...
from engine.crypto.calendar import TIMEFRAME_TO_MS, bars_per_day
...
def _fetch_ohlcv(
    ...
    from engine.crypto.adapters.price_adapter import CryptoPriceAdapter, EXCHANGE_CONFIGS
...
class CryptoService:
    ...
    async def run_backtest(self, request: Any) -> dict:
        ...
        if n_bars < MIN_BARS:
            raise ValueError(
                f"Date range too short: {n_bars} bars (minimum {MIN_BARS}). "
        ...
        if len(ohlcv) < MIN_BARS:
            raise ValueError(
                f"Only {len(ohlcv)} bars available for {request.symbol} "
