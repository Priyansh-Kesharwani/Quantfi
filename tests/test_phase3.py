...
class TestFillPrice:
    ...
    def test_breakdown_keys(self):
        from validation.execution_model import compute_fill_price
...
class TestApplyExecutionCosts:
    ...
    def test_returns_adjusted_series(self, signal_data):
        from validation.execution_model import apply_execution_costs
...
class TestSlippageMatrix:
    def test_returns_dataframe(self):
        from validation.execution_model import slippage_sensitivity_matrix
        np.random.seed(42)
        n = 100
        idx = pd.date_range("2020-01-01", periods=n, freq="B")
        returns = pd.Series(np.random.randn(n) * 0.01, index=idx)
...
class TestExecutionConfig:
    def test_from_dict(self):
        from validation.execution_model import ExecutionConfig
        d = {
...
class TestSearchSpace:
    def test_grid_generation(self):
        from validation.tuning import _generate_grid
        space = {"a": [1, 2], "b": [10, 20]}
    ...
    def test_random_sampling(self):
        from validation.tuning import _sample_random
...
class TestSyntheticLOB:
    def test_generates_lob(self):
        from simulations.hawkes_simulator import simulate_hawkes_events, generate_synthetic_lob
...
class TestSyntheticTrades:
    ...
    def test_empty_events(self):
        from simulations.hawkes_simulator import generate_synthetic_trades
