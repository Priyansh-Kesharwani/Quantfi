...
# Load normalization module directly to avoid pulling in full indicators package (e.g. ldc)
...
def test_expanding_percentile_no_lookahead():
    ...
    # Append one future value (different scale). Percentiles on original index must not change.
    extra = np.array([999999.0])  # arbitrary future value
    series_extra = np.concatenate([series, extra])

    pct_extra, _ = expanding_percentile(series_extra, min_obs=min_obs)

    # Compare on original index [0 .. n-1]
